...
#include "NotUpCheck.h"
#include "UpCheck.h"
#include "Papers.h"
#include "PapersInvalid.h"

CUpLoadProgressBar::CUpLoadProgressBar(void)
	: m_bIdChg(false)
{

}

LPCTSTR CUpLoadProgressBar::GetWindowClassName() const
{ 
	return _T("UILoginWnd"); 
}

UINT CUpLoadProgressBar::GetClassStyle() const
{
	return UI_CLASSSTYLE_DIALOG; 
}

void CUpLoadProgressBar::OnFinalMessage(HWND /*hWnd*/)
{ 
...
//{
//	CUpLoadProgressBar *p = (CUpLoadProgressBar*)dlg;
//
...
void CUpLoadProgressBar::OnPrepare()
{
	//@feng进度条设置
	CProgressUI *pProcessUI1 = static_cast<CProgressUI*>(m_pm.FindControl(_T("UpLoadProgress")));
	pProcessUI1->SetValue(rand()%100);
}

eSocketConnectStatus  CUpLoadProgressBar::Connectd(CStringA Buffer)
{
	WSADATA  Ws;
	SOCKET CientSocket;
...
void CUpLoadProgressBar::ConnectResultProc(eSocketConnectStatus eConnectStatus)
{
...
bool CUpLoadProgressBar::GetIniFile()
{
...
void CUpLoadProgressBar::onLoginBtnClicked()
{
	CString strBuf;
	CComboUI *pComboUI1 = static_cast<CComboUI*>(m_pm.FindControl(_T("field")));
	CComboUI *pComboUI2 = static_cast<CComboUI*>(m_pm.FindControl(_T("proj")));
...
void CUpLoadProgressBar::Notify(TNotifyUI& msg)
{
	TRACE(L"%s\n\n",msg.sType);
	...
	else if( msg.sType == _T("click") )
	{
		...
		if( msg.pSender->GetName() == _T("TEST2") ) 
		{ 
			OnPrepare();
		}
		//@Begin feng二级窗口测试
		if( msg.pSender->GetName() == _T("TEST3") ) 
		{ 
			CNotUpCheckWnd* pNotUpCheckFrame = new CNotUpCheckWnd;
			if(pNotUpCheckFrame == NULL) return;

			pNotUpCheckFrame->Create(this->GetHWND(), NULL, UI_WNDSTYLE_CHILD, 0, 0, 0, 0, 0, NULL);
			pNotUpCheckFrame->ShowWindow(true, false);
			//UINT nResponse = pLoginFrame->ShowModal(); 
		}
		if( msg.pSender->GetName() == _T("TEST4") ) 
		{ 
			CUpCheckWnd* pUpCheckFrame = new CUpCheckWnd;
			if(pUpCheckFrame == NULL) return;

			pUpCheckFrame->Create(this->GetHWND(), NULL, UI_WNDSTYLE_CHILD, 0, 0, 0, 0, 0, NULL);
			pUpCheckFrame->ShowWindow(true, false);
		}
		if( msg.pSender->GetName() == _T("TEST5") ) 
		{ 
			CPapersWnd* pPapersFrame = new CPapersWnd;
			if(pPapersFrame == NULL) return;

			pPapersFrame->Create(this->GetHWND(), NULL, UI_WNDSTYLE_CHILD, 0, 0, 0, 0, 0, NULL);
			pPapersFrame->ShowWindow(true, false);
		}
		if( msg.pSender->GetName() == _T("TEST6") ) 
		{ 
			CPapersInvalidWnd* pPapersInvalidFrame = new CPapersInvalidWnd;
			if(pPapersInvalidFrame == NULL) return;

			pPapersInvalidFrame->Create(this->GetHWND(), NULL, UI_WNDSTYLE_CHILD, 0, 0, 0, 0, 0, NULL);
			pPapersInvalidFrame->ShowWindow(true, false);
		}
		//@End
	}
...
LRESULT CUpLoadProgressBar::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CControlUI* pRoot = builder.Create(_T("UpLoadProgressBar.xml"), 0,NULL , &m_pm);
...
LRESULT CUpLoadProgressBar::OnClose(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	bHandled = FALSE;
	return 0;
}

LRESULT CUpLoadProgressBar::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	//::PostQuitMessage(0L);
...
LRESULT CUpLoadProgressBar::OnNcActivate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if( ::IsIconic(*this) )
		bHandled = FALSE;
...
LRESULT CUpLoadProgressBar::OnNcCalcSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	return 0;
}

LRESULT CUpLoadProgressBar::OnNcPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	return 0;
}

LRESULT CUpLoadProgressBar::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	POINT pt; 
...
LRESULT CUpLoadProgressBar::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CUpLoadProgressBar::OnGetMinMaxInfo(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CUpLoadProgressBar::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CUpLoadProgressBar::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRes = 0;
	BOOL bHandled = TRUE;
	switch( uMsg ) 
...
LRESULT CUpLoadProgressBar::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	if( uMsg == WM_KEYDOWN )
	{
		if( wParam == VK_RETURN )
