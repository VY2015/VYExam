...
CNotUpCheckWnd::CNotUpCheckWnd(void)
	: m_bIdChg(false)
{

}

//CNotUpCheckWnd::CNotUpCheckWnd( CLoginWnd *pParentWnd )
//{
//	m_pParentWnd = pParentWnd;
//}

LPCTSTR CNotUpCheckWnd::GetWindowClassName() const
{ 
	return _T("UIChildLLogin"); 
}

UINT CNotUpCheckWnd::GetClassStyle() const
{
	return UI_CLASSSTYLE_DIALOG; 
}

void CNotUpCheckWnd::OnFinalMessage(HWND /*hWnd*/)
{ 
...
//{
//	CNotUpCheckWnd *p = (CNotUpCheckWnd*)dlg;
//
...
eSocketConnectStatus  CNotUpCheckWnd::Connectd(CStringA Buffer)
{
	WSADATA  Ws;
	SOCKET CientSocket;
...
void CNotUpCheckWnd::ConnectResultProc(eSocketConnectStatus eConnectStatus)
{
...
bool CNotUpCheckWnd::GetIniFile()
{
...
void CNotUpCheckWnd::onLoginBtnClicked()
{
	CString strBuf;
	CComboUI *pComboUI1 = static_cast<CComboUI*>(m_pm.FindControl(_T("field")));
	CComboUI *pComboUI2 = static_cast<CComboUI*>(m_pm.FindControl(_T("proj")));
...
void CNotUpCheckWnd::Notify(TNotifyUI& msg)
{
	TRACE(L"%s\n\n",msg.sType);
...
LRESULT CNotUpCheckWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CControlUI* pRoot = builder.Create(_T("NoUpCheck.xml"), 0,NULL , &m_pm);
...
LRESULT CNotUpCheckWnd::OnClose(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	bHandled = FALSE;
	return 0;
}

LRESULT CNotUpCheckWnd::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	//::PostQuitMessage(0L);
...
LRESULT CNotUpCheckWnd::OnNcActivate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if( ::IsIconic(*this) )
		bHandled = FALSE;
...
LRESULT CNotUpCheckWnd::OnNcCalcSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	return 0;
}

LRESULT CNotUpCheckWnd::OnNcPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	return 0;
}

LRESULT CNotUpCheckWnd::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	POINT pt; 
...
LRESULT CNotUpCheckWnd::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CNotUpCheckWnd::OnGetMinMaxInfo(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CNotUpCheckWnd::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CNotUpCheckWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRes = 0;
	BOOL bHandled = TRUE;
	switch( uMsg ) 
...
LRESULT CNotUpCheckWnd::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	if( uMsg == WM_KEYDOWN )
	{
		if( wParam == VK_RETURN )
