...
// 	ui->SetText(L"aaaaaa");
// 	pComboUI1->Add(ui);
	//@feng初始化创建子窗口ChildLogin
	CChildLoginWnd* pChildLoginFrame = new CChildLoginWnd(this);
	if(pChildLoginFrame == NULL) return ;
	pChildLoginFrame->Create(this->GetHWND(), NULL, UI_WNDSTYLE_CHILD, 0, 250, 160, 0, 0, NULL);
	pChildLoginFrame->ShowWindow(true, false);
	
}

eSocketConnectStatus  CLoginWnd::Connectd(CStringA Buffer)
{
	WSADATA  Ws;
	SOCKET CientSocket;
...
void CLoginWnd::ConnectResultProc(eSocketConnectStatus eConnectStatus)
{
...
void CLoginWnd::onLoginBtnClicked()
{
	CString strBuf;
	CComboUI *pComboUI1 = static_cast<CComboUI*>(m_pm.FindControl(_T("field")));
	CComboUI *pComboUI2 = static_cast<CComboUI*>(m_pm.FindControl(_T("proj")));
...
void CLoginWnd::Notify(TNotifyUI& msg)
{
	...
	else if (msg.sType == _T("link"))
	{
		...
		{ 
			...
			CChildLoginWnd* pChildLoginFrame = new CChildLoginWnd;
			if(pChildLoginFrame == NULL) return;
...
LRESULT CLoginWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CControlUI* pRoot = builder.Create(_T("login.xml"), 0,NULL , &m_pm);
