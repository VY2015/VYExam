...
CChildRegistWnd::CChildRegistWnd(void)
	: m_bIdChg(false)
{

}
CChildRegistWnd::CChildRegistWnd(CLoginWnd *pParentWnd)
{
	m_pRParentWnd = pParentWnd;
}

LPCTSTR CChildRegistWnd::GetWindowClassName() const
{ 
	return _T("UIChildRegist"); 
}

UINT CChildRegistWnd::GetClassStyle() const
{
	return UI_CLASSSTYLE_DIALOG; 
}

void CChildRegistWnd::OnFinalMessage(HWND /*hWnd*/)
{ 
...
//{
//	CChildRegistWnd *p = (CChildRegistWnd*)dlg;
//
...
void CChildRegistWnd::OnPrepare()
{
//	CComboUI *pComboUI1 = static_cast<CComboUI*>(m_pm.FindControl(_T("field")));
...
eSocketConnectStatus  CChildRegistWnd::Connectd(CStringA Buffer)
{
	WSADATA  Ws;
	SOCKET CientSocket;
...
void CChildRegistWnd::ConnectResultProc(eSocketConnectStatus eConnectStatus)
{
...
bool CChildRegistWnd::GetIniFile()
{
...
void CChildRegistWnd::onLoginBtnClicked()
{
	CString strBuf;
	CComboUI *pComboUI1 = static_cast<CComboUI*>(m_pm.FindControl(_T("field")));
	CComboUI *pComboUI2 = static_cast<CComboUI*>(m_pm.FindControl(_T("proj")));
...
void CChildRegistWnd::Notify(TNotifyUI& msg)
{
	TRACE(L"%s\n\n",msg.sType);
	...
	{
		if( msg.pSender->GetName() == _T("Tologin") )
		{ 
			//@feng打开登录子窗口
			CChildLoginWnd* pChildLoginFrame = new CChildLoginWnd(m_pRParentWnd);
			if(pChildLoginFrame == NULL) return;
			pChildLoginFrame->Create(m_pRParentWnd->GetHWND(), NULL, UI_WNDSTYLE_CHILD, 0, 250, 160, 0, 0, NULL);
			pChildLoginFrame->ShowWindow(true, false);
			Close();
...
LRESULT CChildRegistWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	CControlUI* pRoot = builder.Create(_T("ChildRegist.xml"), 0,NULL , &m_pm);
...
LRESULT CChildRegistWnd::OnClose(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	bHandled = FALSE;
	return 0;
}

LRESULT CChildRegistWnd::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	//::PostQuitMessage(0L);
...
LRESULT CChildRegistWnd::OnNcActivate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if( ::IsIconic(*this) )
		bHandled = FALSE;
...
LRESULT CChildRegistWnd::OnNcCalcSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	return 0;
}

LRESULT CChildRegistWnd::OnNcPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	return 0;
}

LRESULT CChildRegistWnd::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	POINT pt; 
...
LRESULT CChildRegistWnd::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CChildRegistWnd::OnGetMinMaxInfo(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CChildRegistWnd::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CChildRegistWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRes = 0;
	BOOL bHandled = TRUE;
	switch( uMsg ) 
...
LRESULT CChildRegistWnd::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	if( uMsg == WM_KEYDOWN )
	{
		if( wParam == VK_RETURN )
